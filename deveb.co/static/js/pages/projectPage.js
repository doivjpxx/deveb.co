...
if(!isMobile){
  ...
  const ogImage = () => {
    if ( newArr.length ) {
      const coverObject = newArr[0].elements.find( ({ type }) => type === 'cover' )

      if ( coverObject ) return `${window.location.origin}${coverObject?.src}` || ''
    }
    return ''
  }
  ...
  return(
    <main className="single-pj" ref={el} id="viewport" data-scroll-container>
       ...
       <Helmet>
        ...
        <meta property="og:title" content={`Deveb | ${title ? title : ""}`} />
        <meta property="og:description" content={`About how we built ${title? title: ""} `} />
